...
def get_sql_data(db: Session, id_set: set, sql_model) -> list:
    ...
    if not data_list:
        raise ColdDataException(status_code=404, detail=f"one or more id are not in {id_set}")
    return data_list
...
def update_multiple_sql_data(db: Session, update_list: list, sql_model):
    try:
        ...
        for row in update_list:
            ...
            if not datum:
                raise ColdDataException(status_code=404, detail=f"id:{row.id} not found")
            for item in row:
...
def delete_multiple_sql_data(db: Session, id_set: set, sql_model):
    try:
        ...
        if len(id_set) != len(delete_data_list):
            raise ColdDataException(status_code=404, detail=f"id: one or many of {str(id_set)} is not exist")
        stmt = delete(sql_model).where(sql_model.id.in_(id_set))
        db.execute(stmt)
        db.flush()
        return delete_data_list
    except UnmappedInstanceError:
        raise ColdDataException(status_code=404, detail=f"id: one or more of {str(id_set)} is not exist")
