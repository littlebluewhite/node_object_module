...
class GeneralOperate(RedisOperate, SQLOperate):
    ...
    def initial_redis_data(self, db_session: sessionmaker):
        db = db_session()
        # sql 取資料
        sql_data = SQLOperate.get_all_sql_data(self, db, self.sql_model)
    ...
    def reload_redis_table(self, db, reload_related_redis_tables, sql_data_list, origin_id_dict=None):
        ...
        if reload_related_redis_tables:
            for table in reload_related_redis_tables:
                ...
                # print("id_set: ", id_set)
                sql_data_list2 = self.reload_redis_from_sql(
    ...
    def reload_redis_from_sql(
            self, db, sql_model, main_schemas, redis_tables: list, data_id_set: set
    ):
        sql_data_list: list = SQLOperate.get_sql_data(self, db, data_id_set, sql_model)
        for table in redis_tables[:1]:
            RedisOperate.write_sql_data_to_redis(
                self, table["name"], sql_data_list, main_schemas, table["key"])
    ...
    def read_data_from_sql_by_id_set(self, db, id_set: set) -> list:
        return SQLOperate.get_sql_data(self, db, id_set, self.sql_model)
